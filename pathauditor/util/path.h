...
#ifndef PATHAUDITOR_UTIL_PATH_H_
#define PATHAUDITOR_UTIL_PATH_H_
...
//
// Usage:
// std::string path = pathauditor::JoinPath("/cns", dirname, filename);
// std::string path = pathauditor::JoinPath(getenv("TEST_SRCDIR"), filename);
//
...
// This is the behavior of the old pathauditor::JoinPath().
// Arguments must be convertible to absl::string_view.
//
// Usage:
// std::string path = pathauditor::JoinPathRespectAbsolute("/f", dirname, filename);
template <typename... T>
...
#endif  // PATHAUDITOR_UTIL_PATH_H_
