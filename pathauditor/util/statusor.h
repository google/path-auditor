...
#ifndef PATHAUDITOR_UTIL_STATUSOR_H_
#define PATHAUDITOR_UTIL_STATUSOR_H_
...
class StatusOr {
 public:
  ...
  T& operator*() & { return this->ValueOrDie(); }
 ...
 private:
  void EnsureOk() const {
    if (!ok()) {
      ...
      PATHAUDITOR_RAW_LOG(
          FATAL, "Attempting to fetch value instead of handling error %s",
          status().message());
    }
  }
...
#endif  // PATHAUDITOR_UTIL_STATUSOR_H_
